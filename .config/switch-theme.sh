...
switch_unified_theme() {
    ...
    case "$theme_name" in
        tokyo|tokyo-night)
            ...
            "$SCRIPT_DIR/.config/tmux/switch-theme.sh" tokyo 2>/dev/null || echo "   tmux: theme not found"
            "$SCRIPT_DIR/switch-starship-theme.sh" tokyo || echo "   starship: theme not found"
            ;;
        catppuccin|mocha)
            echo "🌙 Switching to Catppuccin Mocha theme..."
            "$SCRIPT_DIR/.config/tmux/switch-theme.sh" catppuccin 2>/dev/null || echo "   tmux: theme not found"
            "$SCRIPT_DIR/switch-starship-theme.sh" catppuccin || echo "   starship: theme not found"
            ;;
        gruvbox)
            echo "🌰 Switching to Gruvbox theme..."
            "$SCRIPT_DIR/.config/tmux/switch-theme.sh" original 2>/dev/null || echo "   tmux: theme not found"
